...
class MinimaHopping:
    ...
    def _check_results(self):
        ...
        if self._previous_optimum:
            ...
            self._log('msg', 'Absolute energy difference to last minimum: %.3f eV' % dmax)
    ...
    def _molecular_dynamics(self, N, resume=None):
        ...
        if resume:
            ...
            self._log('msg', 'Starting MD with %i existing energies.' %
                      len(energies))
        if not thermalized:
            self.MaxwellBoltzmannDistribution(self._atoms,N, temp=self._temperature * units.kB, force_temp=True)
        if (self._counter > 1):
            os.remove('md.log')
            os.remove('md.traj')
